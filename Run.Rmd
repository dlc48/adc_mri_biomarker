...
<!--- setwd(.p(.cruk$path,"5412/rmd/2-git/")) --->
...
# colours
.cruk = list(col = c("#EC008C","#00B6ED","#2E008B","#F0B200","#A7A8AA","#3C3C3C"))
...
pdf(.p(path.figures,"F3.pdf"),width=210/18,height=297/18)
...
# rownames
par(mar=c(8,0,0,1),oma=c(0,0,0,0))
    .ep()
    text(.5,.5,"Untreated group",srt=90,cex=2.5)


par(mar=c(8,3,0,2.5))#,oma=c(0,4,0,1)) 
mx.pvalBonf.po = NULL 
for(ow in 1:n.outcome){# ow=1 
    ...
    id.predictor3$col = .cruk$col[3]
    id.predictor3$col[grepl("mu",id.predictor3$id)|
                      grepl("mean",id.predictor3$id)|
                      grepl("median",id.predictor3$id)] = .cruk$col[1]
    #
    xposw = barplot(id.predictor3$gR2,plot=FALSE)
    xlimw = range(xposw)+c(-.75,.75)
    ylimw = c(0,0.4)    
    plot(1,1,pch="",xlim=xlimw,ylim=ylimw,
         xlab = "",ylab="R-squared",
         axes=FALSE,main="")
    barplot(id.predictor3$gR2,add=TRUE,col=id.predictor3$col,axes=FALSE)
    # random
    colw = 1#.p(.cruk$col[3],20)
    gR2_random = id.modelw[grepl("Random",id.modelw$predictor),"gR2"]
    colw = gray(.5)
    segments(xlimw[1],quantile(gR2_random,prob=1),xlimw[2],quantile(gR2_random,prob=1),col=colw, lty=2)
    segments(xlimw[1],quantile(gR2_random,prob=.99),xlimw[2],quantile(gR2_random,prob=.99),col=colw, lty=2)
    segments(xlimw[1],quantile(gR2_random,prob=.95),xlimw[2],quantile(gR2_random,prob=.95),col=colw, lty=2)
    segments(xlimw[1],quantile(gR2_random,prob=.5),xlimw[2],quantile(gR2_random,prob=.5),col=colw, lty=2)
    axis(4,at=quantile(gR2_random,prob=c(1,.99,.95,.5)),
         tick=FALSE,las=2,.p(c(100,99,95,50),"%"),
         col.axis=1,cex.axis=1,hadj=.5)
    # axes
    axis(1,xposw,id.predictor3[,"name2"],cex.axis=1.4,las=2,pos=0)
    axis(1,at=xlimw[2]/2,gsub("\n"," ",id.outcome$name[ow]),
         tick=FALSE,padj=4.5,cex.axis=2,font=2)
    ...
    for(pw in 1:n.predictor){
        ...
        text(xposw[pw],id.predictor3$gR2[pw]+0.025,labels=.sig(id.predictor3$pval[pw]),
             srt=90,col=id.predictor3$col[pw],cex=2)
        }
    }
dev.off()   
```
...
for(ow in 1:n.outcome){# ow=1 
    ...
    id.predictor3$col[grepl("mu",id.predictor3$id)|
                      ...
                      grepl("median",id.predictor3$id)] = .cruk$col[1]    
    #
    xposw = barplot(id.predictor3$R2,plot=FALSE)
    xlimw = range(xposw)+c(-.75,.75)
    ylimw = c(-0.1,0.4)    
    plot(1,1,pch="",xlim=xlimw,ylim=ylimw,
         xlab = "",ylab="R-squared",
         axes=FALSE,main="")
    barplot(id.predictor3$R2,add=TRUE,col=id.predictor3$col,axes=FALSE)
    ...
    axis(4,at=quantile(gR2_random,prob=c(1,.99,.95,.5)),
         ...
         col.axis=1,cex.axis=1,hadj=.5)
    # axes
    axis(1,xposw,id.predictor3[,"name2"],cex.axis=1.4,las=2,pos=-0.1)
    axis(1,at=xlimw[2]/2,gsub("\n"," ",id.outcome$name[ow]),
         tick=FALSE,padj=4.5,cex.axis=2,font=2)
    axis(2,las=2,cex.axis=1)
    axis(2,c(min(ylimw),max(ylimw)),tick=TRUE,labels=c("",""),lwd.ticks=NA)
